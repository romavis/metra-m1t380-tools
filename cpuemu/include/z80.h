...
public:
    ...
    bool on_handle_active_int() {
        ...
        bool irq_allowed;
        if(self().on_is_z80()) {
            irq_allowed = !self().on_is_int_disabled() && self().on_get_iff1();
        } else {
            irq_allowed = !self().on_is_int_disabled() && self().on_get_iff();
        }
        if (irq_allowed) {
            initiate_int();
            accepted = true;
